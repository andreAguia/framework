...
{   
     ...
     * 
     * @group do email
     * @var private $nomeArquivo    string  NULL    O nome do arquivo gerado nesse backup
     */
...
###########################################################    
    ...
    public function backup(){
        ...
        if(count($this->error)>0){
            ...
            if($this->manual){
                $this->nomeArquivo = $pasta.'/'.date('Y.m.d.H.i').'.M.'.$this->dbName;
            }else{
                $this->nomeArquivo = $pasta.'/'.date('Y.m.d.H.i').'.A.'.$this->dbName;
            }

            # Arquivo sql
            $back = fopen($this->nomeArquivo.'.sql','w');    

            # Arquivo txt (relatório)
            $rel = fopen($this->nomeArquivo.'.txt','w');    
